...
int index =0;
while ( (testers[index] != NULL) && (index <= maxNumTesters) ) { // wasnt working after tester removal bc i hadnt made the tester[] = NULL again
...
for (int i = 0; i < currNumTesters; i++) {//if i put maxnumtesters instead wouldnt i be copying nothing sometimes?
if (  (testers[i]->evaluate(num) == true) ) {
    iNumTrues++;
...
return new int[0];//^^oof can we do this? just return a new int[0];
...
*/
//std::cout << "\nNumberTester* TesterInterface::operator[] (int pos)\n";
if (pos>= maxNumTesters)//^^ it doesnt have to be current number of testers right?
{
    return NULL;
}
return testers[pos];
