...
//first create the array to be held 
this->array = new int* [baseSize];//you missed this and after you put it in the entire thing ran without any seg faults
...
////std::cout << "DONE :    this->sizes[i] = sizes[i];\n";
    this->array[i] = new int[ this->sizes[i] ];//this causing a segfault in the child classess

    ////std::cout << "Array Element " <<i << " with new array of size " << this->sizes[i] <<" is being created now\n";
    
    for (int k = 0; k < this->sizes[i]; k++)//^^ sizes[i] is right?
    {
    ////std::cout << "  =================\n"; 
    this->array[i][k]  = array[i][k];
    ////std::cout << i << k << " is " << this->array[i][k] << "\n";
    }
    
    //this->sizes[i] = sizes[i];//umm like, this would only work if se
    ////std::cout << i+1 << " has size = "<<this->sizes[i] << "\n";
}
